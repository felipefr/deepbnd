...
# ns = len(myhd.loadhd5(folder +  'paramRVEdataset_test.hd5', 'param')[:,0])

ns = 2
ids_run = np.arange(run,ns,num_runs).astype('int')
nrun = len(ids_run)

os.system('rm ' + folder +  'snapshots_test_{0}.h5'.format(run))
snapshots, fsnaps = myhd.zeros_openFile(filename = folder +  'snapshots_test_{0}.h5'.format(run),  
...
paramRVEdata = myhd.loadhd5(folder +  'paramRVEdataset_test.hd5', 'param')[ids_run] 
...
for i in range(nrun):
    ...
    meshGMSH.write(folder + "meshes/mesh_temp_{0}.xdmf".format(run), opt = 'fenics')
    
    microModel = micm.MicroModel(folder + "meshes/mesh_temp_{0}.xdmf".format(run), paramMaterial, opModel)
