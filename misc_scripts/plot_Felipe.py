...
folder = 'saves_exFenics_ns2000_Ninout60_lr0001/'

for i in range(1,11):
    histories_tf.append(pickle.load(open(folder + 'historyModel_' + str(i) + '.dat','rb')))
...
plt.subplot('121')
for i in range(10):
    plt.plot(histories_tf[i]['loss'])
    plt.plot(histories_tf[i]['val_loss'])
...
# mean for tf mae
plt.plot(histories_tf_mean['loss'])
plt.plot(histories_tf_mean['val_loss'])
