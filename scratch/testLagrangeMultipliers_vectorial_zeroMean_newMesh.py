...
coeff = coef.getMyCoeff(materials, param, op = 'python')
...
# g = Expression(("sin(3*x[0])*sin(3*x[1])","cos(x[0])*cos(x[1])"), element=V.ufl_element())
...
a = []
a.append([coeff[0]*inner(grad(u), grad(v))*dx, inner(l,v)*ds, inner(p,v)*dx ])
...
a = coeff[0]*inner(grad(u), grad(v))*dx +  inner(p,v)*dx + inner(q,u)*dx
