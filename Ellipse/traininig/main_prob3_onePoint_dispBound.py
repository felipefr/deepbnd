...
mytf.plot_history( history, savefile = partialRadical + 'plot_history_oneMinima_dispBound.png')

with open(partialRadical + 'history_oneMinima_dispBound' + Run_id + '.dat', 'wb') as f:
    pickle.dump(history.history, f)
    
model.save_weights(partialRadical + 'weights_oneMinima_dispBound' + Run_id)
...
mpt.visualiseStresses9x9(scalerY.inverse_transform(Y_t[j:j+9,:]),scalerY.inverse_transform(Y_p[j:j+9,:]), 
                      figNum = j, savefig = partialRadical + '_oneMinimal_dispBound_{0}.png'.format(Run_id))


print(error)
with open(partialRadical + '_oneMinima_dispBound_error_{0}.json'.format(Run_id), 'w') as file:
