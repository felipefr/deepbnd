...
mytf.plot_history( history, savefile = partialRadical + 'plot_history_oneMinima.png')

with open(partialRadical + 'history_oneMinima' + Run_id + '.dat', 'wb') as f:
    pickle.dump(history.history, f)
    
model.save_weights(partialRadical + 'weights_oneMinima' + Run_id)
...
mpt.visualiseStresses9x9(scalerY.inverse_transform(Y_t[j:j+9,:]),scalerY.inverse_transform(Y_p[j:j+9,:]), 
                      figNum = j, savefig = partialRadical + '_oneMinimal_{0}.png'.format(Run_id))


print(error)
with open(partialRadical + '_oneMinima_error_{0}.json'.format(Run_id), 'w') as file:
...
mpt.visualiseScatterErrors(Y_t, Y_p, ['stress Min','x Min','y Min'], gamma = 1.0, 
                           figNum = 7, savefig = partialRadical + '_scatterError_{0}.png'.format(Run_id))
