...
mytf.plot_history( history, savefile = partialRadical + 'plot_history_twoPoints_disp.png')

with open(partialRadical + 'history_twoPoints_disp' + Run_id + '.dat', 'wb') as f:
    pickle.dump(history.history, f)
    
model.save_weights(partialRadical + 'weights_twoPoints_disp' + Run_id)
...
mpt.visualiseStresses9x9(Y_tt[j:j+9,0:3] , Y_pp[j:j+9,0:3] , 
                      figNum = 1, savefig = partialRadical + '_twoPoints_disp_Min_{0}.png'.format(Run_id))

mpt.visualiseStresses9x9( Y_tt[j:j+9,3:] , Y_pp[j:j+9,3:] , 
                      figNum = 2, savefig = partialRadical + '_twoPoints_disp_Max_{0}.png'.format(Run_id))


print(error)
with open(partialRadical + '_twoPoints_disp_error_{0}.json'.format(Run_id), 'w') as file:
...
mpt.visualiseScatterErrors(Y_t[:,0:3], Y_p[:,0:3], ['stress Min','x Min','y Min'], gamma = 1.0, 
                           figNum = 3, savefig = partialRadical + '_scatterError_twoPoints_disp_Min_{0}.png'.format(Run_id))

mpt.visualiseScatterErrors(Y_t[:,3:], Y_p[:,3:], ['stress Max','x Max','y Max'], gamma = 1.0, 
                           figNum = 4, savefig = partialRadical + '_scatterError_twoPoints_disp_Max_{0}.png'.format(Run_id))
