...
def test_buildRVEmesh():
    ...
    buildRVEmesh(paramRVEdata, 'mesh_reduced_temp.xdmf' , isOrdenated = False, size = 'reduced')
    buildRVEmesh(paramRVEdata, 'mesh_full_temp.xdmf' , size = 'full')
    
    vol_targets = {'reduced' : [4.000000000000001, 0.22481976337016096], 
                   'full' : [ 36.0, 0.7963607491654573] }
    
    
    for label in ['reduced', 'full']:
        mesh = EnrichedMesh('mesh_{0}_temp.xdmf'.format(label)) 
