...
def run_pipeline( num_inputs, num_outputs, rb_tol, n_samples, network_type, epochs ):
    ...
    if not status:
        return
    create_folder( name_folder + 'tf' )
    create_folder( name_folder + 'pde' )
    create_folder( name_folder + 'tf_loc' )
    create_folder( name_folder + 'tf_mu' )
    ...
    new_locations_input = np.array(random.sample( range(0, 1000), num_inputs) )
    new_locations_output = np.array(random.sample( range(0, 1000), num_outputs) )
    ...
    errors = np.zeros( (2,3,n_runs) )
    for j in range(n_runs):
        tf_folder = name_folder + 'tf/' + str(j) + '/'
        create_folder( tf_folder )
        start = time.time()
        tf_model, history = etr.build_tensorflow_model( X_train, y_train, epochs, 2, 1, 1, network_width=network_type )
        save_size( tf_folder + "size.txt", tf_model )
        end = time.time()
        write_value_to_file( tf_folder + "rt.txt", end - start )
        save_history( tf_folder, history )

        res = tf_model.predict( X_test )
        save_test_result( tf_folder, res)
        
        err_u = compute_error_on_u( res, y_test, num_outputs )
        
        err_par = compute_errors_on_parameters( res, y_test )
        errors[0,0,j] = err_u
        errors[0,1,j] = err_par[0]
        errors[0,2,j] = err_par[1]
        ...
        err_par = compute_errors_on_parameters( res, y_test )
        errors[1,0,j] = err_u
        errors[1,1,j] = err_par[0]
        errors[1,2,j] = err_par[1]
        
        # consider other tf models
        tf_folder = name_folder + 'tf_loc/' + str(j) + '/'
        create_folder( tf_folder )
        start = time.time()
        tf_model, history = etr.build_tensorflow_model( X_train, y_train[:,:-2], epochs, 2, 0, 1, network_width=network_type )
        save_size( tf_folder + "size.txt", tf_model )
        end = time.time()
        write_value_to_file( tf_folder + "rt.txt", end - start )
        save_history( tf_folder, history )
        ...
        tf_folder = name_folder + 'tf_mu/' + str(j) + '/'
        create_folder( tf_folder )
        start = time.time()
        tf_model, history = etr.build_tensorflow_model( X_train, y_train[:,-2:], epochs, 2, 1, 0, network_width=network_type )
        save_size( tf_folder + "size.txt", tf_model )
        end = time.time()
        write_value_to_file( tf_folder + "rt.txt", end - start )
        save_history( tf_folder, history )
    ...
    np.save( name_folder + 'errors', errors )
        
#create_folder( results_folder )
#in_range = [ 20, 40, 100, 500, 1000 ]
#out_range = [ 20, 40, 100, 500, 1000 ]
#tols = [ 1e-5, 1e-6, 1e-7 ]
#nsamples = [50, 350, 1000, 10000, 20000 ]
#epochs = [20, 50, 100, 200]

#nsamples = [10000, 1000, 20000 ]
#tols = [ 1e-7, 1e-6, 1e-5 ]
    
create_folder( results_folder )
in_range = [ 20, 100, 500 ]
out_range = [ 20, 100, 500 ]
tols = [ 1e-5, 1e-6, 1e-7 ]
nsamples = [10000, 1000, 20000 ]
epochs = [100, 200]
for insamples in nsamples:
    for itols in tols:
        for ioutr in out_range:
            for iin in in_range:
                for iep in epochs:
                    run_pipeline( iin, ioutr, itols, insamples, 'normal', iep )
