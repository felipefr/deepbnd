...
######################################################################
    ...
    with open('./saves_ns20000/historyModel_' + Run_id + '.dat', 'wb') as f:
        pickle.dump(history.history, f)
        
    
    tf_model.save('./saves_ns20000/model_' + Run_id + '.hd5')
    ...
    pde_model,history = etr.build_pde_tensorflow_model( X_train, y_train, EPOCHS, my_pde_activation, 2, weight_mu, \
                                        network_width=chosen_network_width, optimizer='adam' , lr = 0.001)
        
    etr.plot_history( history )
    
    
    with open('./saves_ns20000/historyModel_pde_' + Run_id + '.dat', 'wb') as f:
        pickle.dump(history.history, f)
        
    
    pde_model.save('./saves_ns20000/model_pde_' + Run_id + '.hd5')
