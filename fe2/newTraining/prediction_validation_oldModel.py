...
folderBasisShear = './models/dataset_shear1/' #changed
...
nameScaleXY_shear = folderBasisShear +  '/scaler.txt' # chaged
...
net['nY'] = Nrb
net['nX'] = nX
net['file_weights_shear'] = folderBasisShear + 'models/weights_ny{0}_arch1.hdf5'.format(Nrb)
net['file_weights_axial'] = folderBasisAxial + 'models/weights_ny{0}_arch1.hdf5'.format(Nrb)
...
np.savetxt('randomIndicesTest_200_seed1.txt', randInd)

os.system('rm sigma_prediction_old_ny{0}_eps1.hd5'.format(Nrb))
fields, f = myhd.zeros_openFile('sigma_prediction_old_ny{0}_eps1.hd5'.format(Nrb),
...
for i, ii in enumerate(randInd):  
    ...
    mesh = get_mesh(ellipseData[ii], 'mesh_temp_ny{0}.xdmf'.format(Nrb), 'reduced')    
    meshFull = get_mesh(ellipseData[ii], 'mesh_temp_full_ny{0}.xdmf'.format(Nrb), 'full')
