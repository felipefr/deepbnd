...
DATAfolder = "/Users/felipefr/EPFL/newDLPDES/DATA/deepBoundary/comparisonPODvsDNS/Per/"
nameMeshPrefix = DATAfolder + "RVE_POD_reduced_{0}.{1}"

ntest = 20

stressRad = DATAfolder + 'sigmaL_periodic.hd5'

stressDNS = myhd.genericLoadfile(stressRad,'SigmaL')[:ntest,-1,:]
...
for i in range(ntest):
    ...
    plt.savefig("comparison_POD_test_corrected_test_clean{0}.png".format(i))
    plt.show()
...
plt.legend(loc = 2)
plt.grid()
plt.savefig("comparison_POD_corrected_avg_test_clean.png")
...
plt.legend(loc = 2)
plt.grid()
plt.savefig("comparison_error_POD_corrected_test_clean.png")
...
plt.yscale('log')
plt.grid()
plt.savefig("comparison_error_POD_corrected_test_clean_log.png")
