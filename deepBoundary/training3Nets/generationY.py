...
folder = "/Users/felipefr/EPFL/newDLPDES/DATA/deepBoundary/data3/"
# folder = "/home/felipefr/EPFL/newDLPDEs/DATA/deepBoundary/data1/"
...
EpsFluc = np.loadtxt(folder + 'EpsList_{0}.txt'.format(EpsDirection))
StressList = np.loadtxt(folder + 'sigmaList{0}.txt'.format(EpsDirection))

S = []
for i in range(ns):    
    print('loading simulation ' + str(i) )
    nameMesh = radFile.format('reduced_' + str(i),'xml')
    nameSolution = folder + 'RVE_POD_solution_red_' + str(i) + '_2.h5'
    S.append(recoverFenicsSimulation(nameMesh, nameSolution))
...
for i in range(ns):
    ...
    for j in range(i+1,ns):
        ...
        C[i,j] = assemble(inner(grad(S[i]),grad(ISj))*mesh.dx)
        C[j,i] = C[i,j]


# C = np.loadtxt(folder + 'C_2.txt')[0:400,0:400]
C = C/ns

np.savetxt(folder + 'C_2.txt',C)
...
for nt in range(Ntest):
    ...
    Vref = Uref.function_space()
    meshRef = Vref.mesh()
    
    dxRef = Measure('dx', meshRef) 
    for j in range(Nmax):
        alpha[j] = assemble( inner(bbasis[j], Uref)*meshRef.ds )/meas_ds
        WbasisFuncProj.append(interpolate(WbasisFunc[j],Vref))
        alphaFunc[j] = assemble( inner( grad(WbasisFuncProj[-1]), grad(Uref))*dxRef )
