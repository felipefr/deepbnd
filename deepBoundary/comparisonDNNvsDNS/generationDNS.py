...
for seed in range(0,5):
    ...
    ellipseData_ref[:,2] = fac*ellipseData_ref[:,2]
    ellipseData_ref[:,0] = x0L_ref + fac*(ellipseData_ref[:,0] - x0L)
    ellipseData_ref[:,1] = y0L_ref + fac*(ellipseData_ref[:,1] - y0L)

    meshGMSHredRef = gmsh.ellipseMesh2(ellipseData_ref, x0L_ref, y0L_ref, LxL_ref, LyL_ref , lcar, ifPeriodic)
    meshGMSHredRef.setTransfiniteBoundary(NpLxL)
    meshRedRef = feut.getMesh(meshGMSHredRef, 'reduced_ref_{0}'.format(seed), radFile, createMesh)
