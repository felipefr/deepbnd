...
folder = ["/Users", "/home"][0] + "/felipefr/EPFL/newDLPDEs/DATA/deepBoundary/RBsensibility/veryRegular/"
...
for i in range(ns):
    ...
    for opLoad in ['axial','shear']:
        ...
        for opModel in ['MR','Lin']:
            ...
            U = mpms.solveMultiscale(param, mesh, eps, op = opModel, others = {'bdr' : 4, 'polyorder' : 1, 'per': [x0, x0 + Lxt, y0, y0 + Lyt]})
        
            T, snap_a[opModel + opLoad][i,:], snap_B[opModel + opLoad][i,:,:] = feut.getAffineTransformationLocal(U[0],mesh,[0,1], justTranslation = False)    
        
            usol.interpolate(U[0])
           
            snap_solutions[opModel + opLoad][i,:] = usol.vector().get_local()[:]
            snap_sigmas[opModel + opLoad][i,:] = fmts.homogenisation(U[0], mesh, sigma, [0,1], sigmaEps).flatten()[[0,3,1]]              
    

for f in fsnaps.values():
    f.close()
fellipse.close()
