...
# # suptitle = 'Arch_1_Dataset_Seed_5{0}.{1}' 

# # plt.figure(1)
# # plt.title(suptitle.format('', ''))
# # for i in range(5):
# #     plt.plot(NlistModel, errors[0,:,i,0], '-o', label = 'E2_DNN dataset{0}'.format(i+1))
...
# # plt.grid()
# # plt.legend(loc = 'best')
# # plt.savefig(suptitle.format('_Error','png'))


# suptitle = 'Arch_2_Dataset_Seed_5_maxmin{0}.{1}' 

# plt.figure(1,(8,5))
# plt.title(suptitle.format('', ''))
# for i in range(5):
#     plt.plot(NlistModel, errors[1,:,i,2], '-o', label = 'E2_DNN(max) dataset{0}'.format(i+1))
#     plt.plot(NlistModel, errors[1,:,i,3], '-x', label = 'E2_DNN(min) dataset{0}'.format(i+1))
# plt.plot(np.arange(160),errorPOD,'--', label = 'E2_POD')
# plt.ylim(1.0e-10,0.1)
# plt.xlim(0,200)
# plt.yscale('log')
...
suptitle = 'Historic_Arch_{0}_N{1}_Dataset_Seed_4'.format(arch,Nrb) 


historic_file = './models/newArchitectures_cluster/new4/weights_ny{0}_arch{1}_history.csv'.format(Nrb,arch)
