...
def basicModelTraining(nsTrain, nsVal, nX, nY, net, fnames):
    ...
    ratio_val = nsVal/(nsVal + nsTrain)
     
    indice1 = np.arange(0, nsTrain)
    indice2 = np.arange(nsTrain, len(X))
...
fnames = {}      
fnames['file_weights'] = './models/extendedSymmetry_newCase6/weights_ny{0}_allValidation.hdf5'.format(Nrb)
...
hist = basicModelTraining(nsBlock, nsVal, nX, Nrb, net, fnames)
end = timer()
