...
def getTraining(ns_start, ns_end, scalerX = None, scalerY = None):
    ...
    for i in range(ns_end - ns_start):
        j = i + ns_start
        X[i,:] = np.loadtxt(Xdatafile.format(j))[:4,2]
    
    Y = np.loadtxt("YFunc.txt")[ns_start:ns_end,:] 
...
mytf.plot_history( history, savefile = 'plot_history.png')
