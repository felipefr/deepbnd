...
snapshots, fsnaps = myhd.zeros_openFile(filename = folder +  'snapshots_{0}_{1}.h5'.format(seed,partition),  
                                        shape = [(ntotal,Vref.dim()),(ntotal,3),(ntotal,2),
                                                 (ntotal,2,2)], label = ['solutions','sigma','a','B'], mode = 'w-')

snap_solutions, snap_sigmas, snap_a, snap_B = snapshots


def mirror(ellipseData, perm):
    perm = list(np.array(perm) - 1)
    ellipseData2 = copy.deepcopy(ellipseData)
    ellipseData2[:,2] = ellipseData2[perm,2]
    
    return ellipseData2
    

def mirrorHorizontal(ellipseData):
    perm = [2,1,4,3,8,7,6,5,10,9,12,11,16,15,14,13,22,21,20,19,18,17,24,23,26,25,28,27,30,29,36,35,34,33,32,31]
    return mirror(ellipseData,perm)

def mirrorVertical(ellipseData):
    perm = [3,4,1,2,13,14,15,16,11,12,9,10,5,6,7,8,31,32,33,34,35,36,29,30,27,28,25,26,23,24,17,18,19,20,21,22]
    return mirror(ellipseData,perm)

def mirrorDiagonal(ellipseData):
    return mirrorVertical(mirrorHorizontal(ellipseData))
...
for i, ii in enumerate(range(0,1)):
    print("Solving snapshot", ii)
    start = timer()
    meshGMSH = meut.ellipseMesh2Domains(x0L, y0L, LxL, LyL, NL, mirrorDiagonal(ellipseData[ii,:,:]),
                                        Lxt, Lyt, lcar, x0 = x0, y0 = y0)
    meshGMSH.setTransfiniteBoundary(NpLxt)
    meshGMSH.setTransfiniteInternalBoundary(NpLxL)   
        
    meshGMSH.setNameMesh(folder + "mesh_temp_2.xdmf")
    mesh = meshGMSH.getEnrichedMesh()
    ...
    snap_solutions[i+1,:] = usol.vector().get_local()[:]
    snap_sigmas[i+1,:] = fmts.homogenisation(U[0], mesh, sigma, [0,1], sigmaEps).flatten()[[0,3,2]]        
    end = timer()
    print("concluded in ", end - start)      

    iofe.postProcessing_complete(U[0], folder + 'sol_mesh_2.xdmf', ['u','lame','vonMises'], param)
