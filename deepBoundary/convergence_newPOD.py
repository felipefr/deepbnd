...
for nt in range(Ntest):
    ...
    for j in range(Nmax):
        ...
        WbasisFuncProj.append(interpolate(WbasisFunc[j],V))
        alphaFunc[j] = assemble( inner( WbasisFuncProj[-1], Uref)*mesh.dx )
    ...
    for i,N in enumerate(Nbasis): 
        ...
        for k in range(N):
            UP.vector()[:] = UP.vector()[:] + (alphaFunc[k]/ns)*WbasisFuncProj[k].vector()[:]                
