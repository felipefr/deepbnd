...
S = np.loadtxt('snapshots_MR_balanced.txt')
...
Uref = mpms.solveMultiscale(param, mesh, eps0, op = 'BCdirich_lag', others = [UrefB])
...
input()
# 0.16589035 0.00171492 0.00171492 0.04833216]
# sigma MR: [ 0.1671661  -0.00162329 -0.00162329  0.04699898]
...
for j in range(Nmax):
    ...
    alpha[j] = np.dot(Mh@ Wbasis[:,j], UrefB_dof)/meas_ds
...
for i,N in enumerate(Nbasis):
    ...
    UR_vec = Mhsqrt @ Wbasis[:,:N] @ Wbasis[:,:N].T @  Mhsqrt @ S[:,idEllipse]
    # UR_vec = Wbasis[:,:N] @ Wbasis[:,:N].T @ S[:,idEllipse]
    # UR_vec = S[:,idEllipse]
    
    
    UR = fmts.PointExpression(UR_vec ,g)
    
    UNstrong.append(mpms.solveMultiscale(param[0:2,:], mesh, eps, op = 'BCdirich_lag', others = [UR ]))
    ...
    sigma_i_strong = fmts.homogenisation(UNstrong[-1][0], mesh, sigma1, [0,1], sigmaEps1).flatten()
    
    sigma_i = fmts.homogenisation(UN[i][0], mesh, sigma1, [0,1], sigmaEps1).flatten()
