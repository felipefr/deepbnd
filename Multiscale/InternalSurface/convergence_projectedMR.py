...
Wbasis_, sig_, Mhsqrt_, Mh_ = fmts.pod_customised(S, 'l2', [g,mesh])

# ProjBM = np.eye(len(BM)) - BM @ np.linalg.inv(BM.T @ BM) @ BM.T
# WbasisProj = ProjBM @ Wbasis 

# errorMR = BM.T @ Wbasis
# print('errorMR' , errorMR)

# errorMR = BM.T @ WbasisProj
# print('errorMR' , errorMR)
...
for j in range(Nmax):
    ...
    # alpha[j] = np.dot(Mh@ Wbasis[:,j], MhsqrtInv @ UL_ref_)/meas_ds
...
for i,N in enumerate(Nbasis):
    UR_vec_ = Wbasis_[:,:N] @ Wbasis_[:,:N].T @ UL_ref_
