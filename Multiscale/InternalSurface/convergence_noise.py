...
delta = 0.01
noiseN = 2*delta*(np.random.rand(Ntest,Nmax)-0.5)
noiseR = 2*delta*(np.random.rand(Ntest,Nmax)-0.5)
...
print("Checking homogenisation : ", sigma_ref, StressList[idEllipse,:])
...
alphaR = Wbasis_[:,:Nmax].T @ UL_ref_

for nt in range(Ntest):    
    print(".... Now computing test ", nt)    
    
    alphaN_noise = alpha*(1.0 + noiseN[nt,:])
    alphaR_noise = alphaR*(1.0 + noiseR[nt,:])
    
    for i,N in enumerate(Nbasis): 
        UN = mpms.solveMultiscale(param, mesh, epsL, op = 'POD', others = [bbasis[0:N], alphaN_noise[0:N]])[0]
        
        ULR_vec_ = Wbasis_[:,:N] @ alphaR_noise[:N]
        ...
        sigma_error_R[nt,i] = np.abs(sigma_11_R[nt,i] - sigma_ref[0])
        sigma_error_N[nt,i] = np.abs(sigma_11_N[nt,i] - sigma_ref[0])
    ...
    sigma_error_N_rel[nt,:] = sigma_error_N[nt,:]/sigma_ref[0]
    sigma_error_R_rel[nt,:] = sigma_error_R[nt,:]/sigma_ref[0]
    normUN_Uex_L2_rel[nt,:] = normUN_Uex_L2[nt,:]/normUex_L2
    normUR_Uex_L2_rel[nt,:] = normUR_Uex_L2[nt,:]/normUex_L2
    normUN_Uex_L2domain_rel[nt,:] = normUN_Uex_L2domain[nt,:]/normUex_L2domain
    normUR_Uex_L2domain_rel[nt,:] = normUR_Uex_L2domain[nt,:]/normUex_L2domain
    normUN_Uex_Rm_rel[nt,:] = normUN_Uex_Rm[nt,:]/normUex_Rm
    normUR_Uex_Rm_rel[nt,:] = normUR_Uex_Rm[nt,:]/normUex_Rm
...
plt.xlabel('N')
plt.ylabel('sigma_11')
plt.plot([Nbasis[0],Nbasis[-1]], 2*[sigma_ref[0]],'k', label = 'stress reference')
