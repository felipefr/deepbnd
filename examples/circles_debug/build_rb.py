...
def translateSolution(nameSnaps, Vref):
    for load_flag in ['A', 'S']:
        ...
        myhd.addDataset(fIsol,Isol_trans, 'solutions_trans_partial_2_%s'%load_flag)
        fIsol.close()
...
def computingBasis(nameSnaps, nameWbasis, Nmax, Nh, Vref, dsRef):
    ...
    for load_flag, Wbasis, sig in zip(['A', 'S'],[Wbasis_A , Wbasis_S], [sig_A,sig_S]):
        Isol = myhd.loadhd5(nameSnaps,'solutions_trans_partial_2_%s'%load_flag)
...
def createXY(nameParamRVEdataset, nameYlist, nameXYlist, id_feature = 2, transformX = lambda x: x):
    
    os.system('rm ' + nameXYlist)
    Y = myhd.loadhd5(nameYlist,['Ylist_%s'%s for s in ['A','S']])
    X = transformX(myhd.loadhd5(nameParamRVEdataset,'param')[:,:, id_feature])
...
if __name__ == '__main__': 
    ...
    folder = "/home/felipe/deepBND/DATA/DEBUG/dataset_recreation/"
    folder_mesh = "/home/felipe/deepBND/DATA/DEBUG/dataset_recreation/"
    ...
    if(op==0):
        ...
        createXY(nameParamRVEdataset, nameYlist, nameXYlist, id_feature = 2)
