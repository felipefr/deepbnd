...
def inverse_meanCompliance(compliance):
    
    mean_compliance = np.zeros((3,3))
    for i in range(3):
        for j in range(3):
            mean_compliance[i,j] = np.mean(compliance[:,i,j])
...
for case in cases:
    if(case == 'HF_large'):
        ...
        compliance[case] = np.array( [ np.linalg.inv(sym(tangents[case][i])) for i in range(len(tangents[case])) ]) 
...
for case in cases[:-1]:
    ...
    for i in range(ns):
        errors[case][i] = np.linalg.norm(compliance[case][i] - compliance[refCase][i])
        errors_rel[case][i] = errors[case][i]/np.linalg.norm(compliance[refCase][i])
...
for case in cases:
    ...
    compliance_avg[case] = np.zeros((NS,3,3))
    ...
    for i in range(NS):
        ...
        # tangents_avg[case][i,:,:] = inverse_meanCompliance( compliance[case][selected_ids,:,:])
        
tangents_full = myhd.loadhd5(snapshotsFullName, 'tangentL')
compliance_full = np.array( [ np.linalg.inv(sym(tangents_full[i])) for i in range(len(tangents_full)) ]) 
...
for case in cases:
    ...
    avgCompliance_subdomains_N[case] = np.zeros(ns)
    ...
    for i in range(ns):
        ...
        avgCompliance_subdomains_N[case][i] = inverse_meanCompliance(compliance[case][ shuffled_ids[: i+1]])[ind1, ind2]
...
plt.plot(np.arange(NS0, NS) + 1, avgCompliance_N[NS0:], '-o', label = 'full Compliance')
for case in cases:
    plt.plot(fac*(np.arange(ns0, ns) + 1), avg_subdomains_N[case][ns0:], '-o', label = case)
    plt.plot(fac*(np.arange(ns0, ns) + 1), avgCompliance_subdomains_N[case][ns0:], '-o', label = case + "compliance")
...
meanSqrt = lambda a, b : np.sqrt(a*b)
meanHarm = lambda a, b : 2./((1./a) + (1/b))

plt.figure(4)
plt.plot(np.arange(NS0, NS) + 1, meanHarm(avgCompliance_N[NS0:],avg_N[NS0:]), '-o', label = 'full')
for case in cases:
    plt.plot(fac*(np.arange(ns0, ns) + 1), meanHarm(avgCompliance_subdomains_N[case][ns0:], avg_subdomains_N[case][ns0:]), '-o', label = case)
