...
avg_eig_N = np.zeros(NS)
std_eig_N = np.zeros(NS)
cfi_eig_N = np.zeros(NS)

timeFull_N = np.linspace(timeFull, NS*timeFull, NS)


for i in range(NS):
    avg_N[i] = np.mean( tangents_full[ : i+1, ind1, ind2] )
    std_N[i] = np.std( tangents_full[ : i+1, ind1, ind2] )
    cfi_N[i] = 1.96*std_N[i]/(np.sqrt(i+1)*avg_N[i]) # i + 1 is the actual number

    avg_eig_N[i] = np.mean( eigenvalues_full[ : i+1, ind_eig] )
    std_eig_N[i] = np.std( eigenvalues_full[ : i+1, ind_eig] )
    cfi_eig_N[i] = 1.96*std_eig_N[i]/(np.sqrt(i+1)*avg_eig_N[i]) # i + 1 is the actual number
...
avg_eig_subdomains_N = {}
std_eig_subdomains_N = {}
cfi_eig_subdomains_N = {}
...
for case in cases:
    ...
    avg_eig_subdomains_N[case] = np.zeros(ns)
    std_eig_subdomains_N[case] = np.zeros(ns)
    cfi_eig_subdomains_N[case] = np.zeros(ns)
    ...
    for i in range(ns):
        ...
        avg_eig_subdomains_N[case][i] = np.mean( eigenvalues[case][ shuffled_ids[: i+1], ind_eig] )
        std_eig_subdomains_N[case][i] = np.std( eigenvalues[case][ shuffled_ids[: i+1], ind_eig] )
        cfi_eig_subdomains_N[case][i] = 1.96*std_eig_subdomains_N[case][i]/(np.sqrt(i+1)*avg_eig_subdomains_N[case][i]) # i + 1 is the actual number
...
plt.figure(1)
plt.plot(np.arange(NS0, NS) + 1, avg_N[NS0:], '-o')
for case in cases:
...
plt.figure(2)
plt.plot(np.arange(NS0, NS) + 1, cfi_N[NS0:], '-o')
for case in cases:
...
# plt.figure(5)
# plt.plot(np.arange(NS0, NS) + 1, avg_eig_N[NS0:], '-o')
# for case in cases:
#     plt.plot(fac*(np.arange(ns0, ns) + 1), avg_eig_subdomains_N[case][ns0:], '-o', label = case)

# plt.legend()
# plt.grid()

# plt.figure(6)
# plt.plot(np.arange(NS0, NS) + 1, cfi_eig_N[NS0:], '-o')
# for case in cases:
#     plt.plot(fac*(np.arange(ns0, ns) + 1), cfi_eig_subdomains_N[case][ns0:], '-o', label = case)
