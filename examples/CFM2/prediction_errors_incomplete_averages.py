...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 29 17:29:10 2022
...
def compute_DNN_error(net, Ylabel, Xmask):
    ...
    if(type(Xmask) != type(None)):
        ...
        for j in range(n_samples):
            Xbars.append(np.zeros(Xbar.shape))
            Xbars[j][:, Xmask] = Xbar[:, Xmask]
            for i in lacking:
                Xbars[j][:, i] = 0.5*(np.max(Xbar[:,i]) + np.min(Xbar[:,i])) + delta_noise*np.random.randn(len(Xbar[:,0]))
    ...
    Yp = np.zeros(Ybar.shape)
    for i in range(n_samples):
        Yp[:,:] =  Yp[:,:]  + scalerY.inverse_transform(model.predict(Xbars[i]))
...
if __name__ == '__main__':
    ...
    net.files['scaler'] = folderTrain + 'scalers_%s.txt'%(load_flag)
